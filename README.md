...
| **AI**      | OpenAI Realtime API · Chat Completions API |
| **DB**      | Firebase Firestore (편지) · JSON (로컬 세이브) |
...
| Firebase 프로젝트           | 편지 데이터를 저장할 Firestore 데이터베이스입니다. 설정 방법은 아래 **3. Firebase 설정** 절에서 설명합니다. |
...
### 3. Firebase 설정 (Firestore 데이터베이스)

편지 시스템의 데이터(플레이어가 작성한 편지와 부모 NPC의 답장)는 **Firebase Firestore** 클라우드 데이터베이스의 `letters` 컬렉션에 문서 단위로 저장됩니다. 처음부터 구축하는 경우 [Firebase 콘솔](https://console.firebase.google.com/)에서 프로젝트를 만든 뒤, 아래 두 가지 자격 증명 파일을 준비해야 합니다. 두 파일 모두 API 키를 포함하고 있어 `.gitignore` 대상이므로 저장소에 포함되지 않으며, 각자 직접 발급·배치해야 합니다.

**(1) 서버용 — Admin SDK 서비스 계정 키.**
AI 서버는 Firebase Admin SDK 서비스 계정 키(`gogo-golem-firebase-adminsdk-fbsvc-*.json`)를 사용해 Firestore에 접근합니다. 이 키는 Firebase 콘솔의 **프로젝트 설정 → 서비스 계정 → 새 비공개 키 생성** 메뉴에서 발급할 수 있으며, `GoGoGolem/src/ai/` 경로에 배치한 뒤 `.env` 파일의 `FIREBASE_CREDENTIALS_PATH` 에서 해당 경로를 지정합니다. (위 **2. AWS EC2 구축** 의 6단계에서 이미 업로드했다면 생략해도 됩니다.)

**(2) 클라이언트용 — google-services.json.**
Unity 클라이언트는 `Assets/google-services.json` 에 포함된 프로젝트 설정을 이용해 Firestore에 연결합니다. 이 파일은 Firebase 콘솔에서 앱을 등록할 때 내려받는 클라이언트용 설정 파일이며, 발급·배치 방법은 아래 **Firebase Unity SDK 설치 가이드** 에서 자세히 설명합니다.
...
> **로컬 세이브 데이터**: 퀘스트 진행 상태, 인벤토리, 숙소(방) 상태처럼 클라우드 동기화가 필요하지 않은 게임 데이터는 **JSON 파일** 형태로 클라이언트 로컬에 저장됩니다. 별도 설정 없이 게임 실행 시 자동으로 생성·관리됩니다.

### 4. MediaPipe Unity Plugin
...
### 5. Unity Asset Store 에셋 임포트

게임에 사용한 3D 모델·텍스처·사운드·VFX 중 상당수는 **Unity Asset Store 에셋** 으로, 라이선스상 저장소에 포함할 수 없어 `.gitignore` 로 제외되어 있습니다. 따라서 클론한 프로젝트를 그대로 열면 일부 씬(특히 `Forest`, `Gesture Detection Wind`)에서 모델·머테리얼이 누락되어 분홍색으로 표시될 수 있습니다.

모든 씬을 정상적으로 표시하려면, 아래 [사용 에셋](#사용-에셋) 표에 정리된 에셋들을 각자 [Unity Asset Store](https://assetstore.unity.com/)(또는 표에 명시된 출처)에서 본인 계정으로 내려받아 프로젝트에 임포트해야 합니다. 각 에셋이 사용된 씬과 위치는 [사용 에셋](#사용-에셋) 표를 참고하세요.

### 6. Unity 클라이언트에 AI 서버 주소 연결
...
16팀 트리오링고 **강한나** **박세은** **최은별**


## 키워드

`#아동게임` `#멀티모달AI` `#사회적상호작용` `#스토리` `#어드벤처`
