...
namespace Demo.GestureDetection
{
  public class AvatarLandmarkAnimator : MonoBehaviour
  {
    ...
    // 외부(Runner)에서는 이 함수를 통해 데이터만 넣어줍니다.
    ...
    // [핵심 변경] 실제 움직임 처리는 Animator가 끝난 뒤인 LateUpdate에서 수행
    private void LateUpdate()
    {
      ...
      // ✨ 3. 캐시된 회전값으로 부드럽게 보간 (flickering 방지의 핵심)
    ...
    private void ProcessMovement(HandLandmarkerResult result)
    {
      ...
      if (result.handedness != null && result.handedness.Count >= 2)
      {
          ...
          // 여기서는 기존 로직대로 label0 == "Right"이면 hand0이 왼손 데이터라고 가정
          ...
          {
              // 거울 모드가 아닐 경우 (좌우 반전)
              // 왼손 타겟에 오른손 데이터를 넣거나 하는 로직 (기존 로직 유지)
              ...
              if (isHand0Left) {
                  ...
                  UpdateHandRotation(hand0Marks, _rightHandTarget, false); // ★ 추가
                  ...
                  UpdateHandRotation(hand1Marks, _leftHandTarget, true); // ★ 추가
                  UpdateFingerTargets(hand1Marks, _leftHandFingers, true);
              } else {
                  // Hand0(Right) -> Left Target
                  UpdateArmIK(poseLandmarks[11], poseLandmarks[13], poseLandmarks[15], _leftHandTarget, _leftElbowHint, true);
                  UpdateHandRotation(hand0Marks, _leftHandTarget, true); // ★ 추가
                  UpdateFingerTargets(hand0Marks, _leftHandFingers, true);

                  // Hand1(Left) -> Right Target
                  UpdateArmIK(poseLandmarks[12], poseLandmarks[14], poseLandmarks[16], _rightHandTarget, _rightElbowHint, false);
                  UpdateHandRotation(hand1Marks, _rightHandTarget, false); // ★ 추가
    ...
    // ✨ 캐시된 회전값으로 부드럽게 적용 (flickering 방지의 핵심)
    private void ApplyCachedFingerRotations()
    {
      foreach (var kvp in _targetFingerRotations)
      {
        ...
        if (Quaternion.Angle(currentRotWorld, targetRotWorld) > _minRotationThreshold)
        {
          ...
          // ✨ 월드 회전 → 로컬 회전으로 변환하여 적용
          // 이렇게 해야 부모(손목) 회전이 자동으로 반영됨!
      ...
      bool isLeftArm)
    {
      ...
      /*
      float additionalXOffset = isLeftArm ? -0.2f : 0.2f;
      shoulderToWrist.x += additionalXOffset;*/
      ...
      if (elbowHint != null)
      {
        ...
        // shoulderToElbow.x += additionalXOffset * 0.3f; 
        ...
        bool isLeftHand)
    {
        ...
        // 중지(9) 대신 검지와 새끼의 중점을 사용하여 더 안정적인 '손의 중심 방향'을 찾습니다.
