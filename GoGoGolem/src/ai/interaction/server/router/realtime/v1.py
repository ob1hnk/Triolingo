...
from interaction.server.dto.realtime import RealtimeMessageType as MessageType, QuestContext
        ...
        while True:
            ...
            try:
                if message_type == MessageType.STREAM_START:
                    ...
                    # 퀘스트 컨텍스트 파싱 (없으면 None)
                    quest_context: Optional[QuestContext] = None
                    if "quest_context" in data and data["quest_context"] is not None:
                        try:
                            quest_context = QuestContext.model_validate(data["quest_context"])
                            logger.info(
                                f"Quest context received: {len(quest_context.active_quests)} active, "
                                f"{len(quest_context.completed_quest_ids)} completed quests"
                            )
                        except Exception as e:
                            logger.warning(f"Failed to parse quest_context: {e}")
